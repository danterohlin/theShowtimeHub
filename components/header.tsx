...
export default function header({ setShowsData, publicRuntimeConfig, initialData }: any) {
    ...
    return (
        <div className='bg-slate-800 w-full border-b border-slate-700'>
            <div className="flex flex justify-between px-8 max-w-8xl m-auto items-center text-white">
                <div className="w-1/3"><h1 onClick={reset} className="text-2xl font-bold cursor-pointer font-nunito ">The Showtime <span className="text-sky-400">Hub</span></h1></div>
                <form onSubmit={search} className="flex p-4 w-1/3">
                    <input className="search bg-transparent bg-slate-700 w-96 focus:outline-0 rounded-l-xl px-4 py-1  " name="searchTerm" type="text" value={searchTerm} onChange={handleInput} />
                    <button className="btn inline-block px-6 py-2.5 bg-sky-600 text-white font-medium text-xs leading-tight uppercase rounded-r-xl shadow-md hover:bg-sky-700 hover:shadow-lg focus:bg-blue-700  focus:shadow-lg focus:outline-none focus:ring-0 active:bg-sky-800 active:shadow-lg transition duration-150 ease-in-out flex items-center" id="button-addon2" type="submit">
